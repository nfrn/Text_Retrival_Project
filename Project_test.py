...
from keras.layers import Dense, Input, LSTM, Embedding, concatenate,Reshape
from nltk.corpus import wordnet as wn
import datetime
import csv

#Which subtask(1 or 2 or 3) to run and with or without training(0 or 1)
SUBTASK = 3
TRAIN = 1
...
def load_data(path):
    ...
    data, target, images = split_data(encoded_sentences)
    ...
    return embeddings, reverse_word_map, data, answersT1, voc_size, questions, answers, answersT0, images
...
def createModel3(embeddings, voc_size):
    ### build encoder
    ...
    ### build decoder
    image_features_input = Input(shape=(IMG_FEA_TOTAL,), dtype='float', name='image_features_input')
    activate_features = Dense(IMG_FEA_TOTAL, input_shape=(IMG_FEA_TOTAL,))(image_features_input)

    dec_sequence = LSTM(IMG_FEA_TOTAL, return_sequences=True, input_shape=(None, MAX_WORDS_QUESTION))(activate_features, enc_states)

    decoder_dense = Dense(voc_size, activation=ACTIVATION)(dec_sequence)

    ### build model
    model = Model(input=[enc_input, image_features_input], output=decoder_dense)
    model.compile(optimizer=OPTIMIZER, loss=LOSS_FUNCTION, metrics=['accuracy'])
...
def get_images_indexes(reverse_word_map,images, image_dict):
    questions_image_features = np.zeros((len(images),IMG_FEA_TOTAL))
    for idx,image in enumerate(images):
        questions_image_features[idx] = image_dict[reverse_word_map[image]]
    return questions_image_features



def main():
    embeddings, reverse_word_map, data, target, voc_size, questions, answers, answerst1, images = load_data(TRAIN_DATA)

    image_dict = loadImageFeatures() # dic["imageXXX"] =[features...]

    question_image_features = get_images_indexes(reverse_word_map,images, image_dict)
    print(question_image_features.shape)
    ...
    if SUBTASK == 1:
        ...
        data = data.reshape(data.shape[0], MAX_WORDS_QUESTION)
        model = createModel3(embeddings, voc_size)
